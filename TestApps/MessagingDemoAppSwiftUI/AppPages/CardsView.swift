...
struct CardsView: View, ContentCardUIEventListening {
    ...
    // Custom XDM input — exercises updatePropositionsForSurfaces(_:withXdm:andData:)
    ...
    private var actionPanel: some View {
        VStack(spacing: 10) {
            ...
            // Custom XDM controls — updatePropositionsForSurfaces(_:withXdm:andData:)
            customXdmPanel
    ...
    // MARK: - Custom XDM Panel

    /// Collects a JSON XDM payload and sends it via the custom-XDM overload of
    /// `updatePropositionsForSurfaces(_:withXdm:andData:)` for `cardsSurface`.
    private var customXdmPanel: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Custom XDM (JSON)")
                .font(.system(size: 11, weight: .semibold))
                .foregroundColor(.secondary)
            ...
            actionButton(title: "Update prop with XDM", systemImage: "arrow.down.circle.badge.clock") {
                updatePropWithXdm()
            }
        }
        .padding(12)
    ...
    /// Parses the JSON in `xdmInput` and calls the custom-XDM overload of
    /// `updatePropositionsForSurfaces(_:withXdm:andData:)` for `cardsSurface`.
    func updatePropWithXdm() {
        xdmParseError = ""
        let trimmed = xdmInput.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty,
              let data = trimmed.data(using: .utf8),
              let xdm = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            xdmParseError = "⚠️ Invalid or empty JSON — check XDM input"
            return
        }

        statusMessage = "Updating propositions for cardsSurface with custom XDM…"
        Messaging.updatePropositionsForSurfaces([cardsSurface], withXdm: xdm, andData: nil) { success in
            DispatchQueue.main.async {
                statusMessage = success
                    ? "updatePropositionsForSurfaces(withXdm:) succeeded for cardsSurface"
                    : "updatePropositionsForSurfaces(withXdm:) failed for cardsSurface"
            }
        }
    }
